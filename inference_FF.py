# Save this as inference_FF later
import numpy as np
...
import torch
from models import Landmark_generator_FF
...
parser.add_argument('--main_audio', type=str, default='./test/template_video/audio2.wav')
parser.add_argument('--other_audio', type=str, default='./test/template_video/audio2.wav')
...
parser.add_argument('--landmark_gen_checkpoint_path', type=str, default='./test/checkpoints/landmarkgenerator_multi_FF.pth')
...
input_main_audio_path = args.main_audio
input_other_audio_path = args.other_audio
...
outfile_path = os.path.join(output_dir,
                       '{}_N_{}_Nl_{}_FF.mp4'.format(input_video_path.split('/')[-1][:-4] + 'result', ref_img_N, Nl))
if os.path.isfile(input_video_path) and input_video_path.split('.')[1] in ['jpg', 'png', 'jpeg']:
    args.static = True
...
landmark_generator_model = load_model(
    model=Landmark_generator_FF(T=T, d_model=512, nlayers=4, nhead=4, dim_feedforward=1024, dropout=0.1),
...
##(2) Extracting audio####
if not input_main_audio_path.endswith('.wav'):
    command = 'ffmpeg -y -i {} -strict -2 {}'.format(input_main_audio_path, '{}/temp_main.wav'.format(temp_dir))
    subprocess.call(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    input_main_audio_path = '{}/temp_main.wav'.format(temp_dir)
main_wav = audio.load_wav(input_main_audio_path, 16000)
main_mel = audio.melspectrogram(main_wav)  # (H,W)   extract mel-spectrum
if not input_other_audio_path.endswith('.wav'):
    command = 'ffmpeg -y -i {} -strict -2 {}'.format(input_other_audio_path, '{}/temp_other.wav'.format(temp_dir))
    subprocess.call(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    input_other_audio_path = '{}/temp_other.wav'.format(temp_dir)
other_wav = audio.load_wav(input_other_audio_path, 16000)
other_mel = audio.melspectrogram(other_wav)  # (H,W)   extract mel-spectrum

assert main_mel.shape == other_mel.shape, f'The shape of main_mel is {main_mel.shape} but other_mel is {other_mel.shape}.'

##read audio mel into list###
main_mel_chunks = []  # each mel chunk correspond to 5 video frames, used to generate one video frame
other_mel_chunks = []  # each mel chunk correspond to 5 video frames, used to generate one video frame
mel_idx_multiplier = 80. / fps
mel_chunk_idx = 0
while 1:
    start_idx = int(mel_chunk_idx * mel_idx_multiplier)
    if start_idx + mel_step_size > len(main_mel[0]):
        break
    main_mel_chunks.append(main_mel[:, start_idx: start_idx + mel_step_size])  # mel for generate one video frame
    other_mel_chunks.append(other_mel[:, start_idx: start_idx + mel_step_size])  # mel for generate one video frame
    mel_chunk_idx += 1
...
input_mel_chunks_len = len(main_mel_chunks)
...
for batch_idx, batch_start_idx in tqdm(enumerate(range(0, input_mel_chunks_len - 2, 1)),
    ...
    T_main_mel_batch, T_other_mel_batch, T_crop_face,T_pose_landmarks = [], [], [], []
    ...
    for mel_chunk_idx in range(batch_start_idx, batch_start_idx + T):  # for each T frame
        # 1 input audio
        T_main_mel_batch.append(main_mel_chunks[max(0, mel_chunk_idx - 2)])
        T_other_mel_batch.append(other_mel_chunks[max(0, mel_chunk_idx - 2)])
  # +frame_h*3
    ...
    T_main_mels = torch.FloatTensor(np.asarray(T_main_mel_batch)).unsqueeze(1).unsqueeze(0)  # 1,T,1,h,w
    T_other_mels = torch.FloatTensor(np.asarray(T_other_mel_batch)).unsqueeze(1).unsqueeze(0)  # 1,T,1,h,w
    ...
    T_main_mels, T_other_mels, T_pose = T_main_mels.cuda(), T_other_mels.cuda(), T_pose.cuda()
    with torch.no_grad():  # require    (1,T,1,hv,wv)(1,T,2,74)(1,T,2,57)
        predict_content = landmark_generator_model(T_main_mels, T_other_mels, T_pose, Nl_pose, Nl_content)  # (1*T,2,57)
    ...
    if batch_idx == 0:
        ...
        out_stream.write(full)
out_stream.release()
command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(input_main_audio_path, '{}/result.avi'.format(temp_dir), outfile_path)
subprocess.call(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
