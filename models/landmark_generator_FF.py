# Revised file of landmark_generator.py
# Save this as landmark_generator_multi.py after we get GPU
import torch
...
class Track_combiner(nn.Module):
    def __init__(self, T, act='tanh'):
        super(Track_combiner, self).__init__()
        self.T = T
        self.track = nn.Embedding(3, 512, padding_idx=0)  # 1 for track 1(main),  2 for track 2
    ...
    def forward(self, main_mel, other_mel):
        # track encoding
        track_main = self.track(1 * torch.ones((main_mel.size(0), self.T), dtype=torch.int).cuda())
        track_other = self.track(2 * torch.ones((other_mel.size(0), self.T), dtype=torch.int).cuda())
        main_mel_encoded = main_mel + track_main
        other_mel_encoded = other_mel + track_other
        combined_mel = torch.cat((main_mel_encoded, other_mel_encoded), dim=-1)
        combined_mel = self.linear(combined_mel)
        combined_mel = self.act(combined_mel)
        combined_mel = combined_mel + main_mel # Residual
        return combined_mel
...
class Landmark_generator_FF(nn.Module):
    def __init__(self,T,d_model,nlayers,nhead,dim_feedforward,dropout=0.1):
        ...
        self.other_mel_encoder=nn.Sequential(  #  (B*T,1,hv,wv)
            Conv2d(1, 32, kernel_size=3, stride=1, padding=1),
            Conv2d(32, 32, kernel_size=3, stride=1, padding=1, residual=True),
            Conv2d(32, 32, kernel_size=3, stride=1, padding=1, residual=True),
        ...
        self.track_combiner = Track_combiner(T)
    ...
    def forward(self, T_mels_main, T_mels_other, T_pose, Nl_pose, Nl_content):
        ...
        # 2. get embedding
        main_mel_embedding = self.main_mel_encoder(T_mels_main).squeeze(-1).squeeze(-1) #(B*T,512)
        other_mel_embedding = self.other_mel_encoder(T_mels_other).squeeze(-1).squeeze(-1) #(B*T,512)
        pose_embedding = self.pose_encoder(T_pose).squeeze(-1)  # (B*T,512)
        ref_embedding = self.ref_encoder(Nl_ref).squeeze(-1)  # (B*Nl,512)
        #normalization
        main_mel_embedding = self.Norm(main_mel_embedding)  # (B*T,512)
        other_mel_embedding = self.Norm(other_mel_embedding)  # (B*T,512)
        pose_embedding =self.Norm(pose_embedding)   # (B*T,512)
        ref_embedding =  self.Norm(ref_embedding) # (B*Nl,512)

        main_mel_embedding = torch.stack(torch.split(main_mel_embedding,T),dim=0) #(B,T,512)
        other_mel_embedding = torch.stack(torch.split(other_mel_embedding,T),dim=0) #(B,T,512)
        ...
        #3. combine tracks
        mel_embedding=self.track_combiner(main_mel_embedding,other_mel_embedding)
