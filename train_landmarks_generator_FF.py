# Save this as train_landmarks_generator_FF.py
import os
...
Nl=15
T = 5
Project_name = f'landmark_FF_T{T}_d{d_model}_fe{dim_feedforward}_lay{nlayers}_head{nhead}'
print('Project_name:', Project_name)
finetune_path = args.finetune_path
num_workers = 8
batch_size =32 #128  # 512
batch_size_val =32 #128  #512
evaluate_interval = 500 #5000  #
...
class Dataset(object):
    ...
    def __getitem__(self, idx):
        while 1:
            ...
            # 03. get T audio
            try:
                main_audio_mel = np.load(join(args.pre_audio_root,vid_name, "audio_matched.npy"))
                other_audio_mel = np.load(join(args.pre_audio_root,vid_name, "audio_other.npy"))
            ...
            for frame_idx in T_idxs:
                ...
                main_m = main_audio_mel[start_idx: start_idx + mel_step_size, :]  # get five frames around
                other_m = other_audio_mel[start_idx: start_idx + mel_step_size, :]
                if main_m.shape[0] != mel_step_size or other_m.shape[0] != mel_step_size:  # in the end of vid
                    break
                T_main_mels.append(main_m.T)  # transpose
                T_other_mels.append(other_m.T)
            if len(T_main_mels) != T:
                continue
            T_main_mels = np.asarray(T_main_mels)  # (T,hv,wv)
            T_main_mels = torch.FloatTensor(T_main_mels).unsqueeze(1)  # (T,1,hv,wv)
            T_other_mels = np.asarray(T_other_mels)  # (T,hv,wv)
            T_other_mels = torch.FloatTensor(T_other_mels).unsqueeze(1)  # (T,1,hv,wv)
            #  return value
            return T_main_mels, T_other_mels,    T_pose,   T_content,Nl_pose,Nl_content
            #     (T,1,hv,wv)   (T,1,hv,wv)      (T,2,74)  (T,2,57)

def load_checkpoint(path, model, optimizer, reset_optimizer=False, overwrite_global_states=True, freeze=True):
    global global_step
    global global_epoch
...
def evaluate(model, val_data_loader):
    ...
    for epoch in tqdm(range(eval_epochs),total=eval_epochs):
        ...
        for step, (T_main_mels, T_other_mels, T_pose,T_content,Nl_pose,Nl_content) in prog_bar:
            model.eval()
            T_main_mels, T_other_mels, T_pose, T_content,Nl_pose,Nl_content = T_main_mels.cuda(non_blocking=True), T_other_mels.cuda(non_blocking=True), \
                                                                 T_pose.cuda(non_blocking=True), T_content.cuda(non_blocking=True), \
                                                                 Nl_pose.cuda(non_blocking=True), Nl_content.cuda(non_blocking=True)
            # (B,T,1,hv,wv) (B,T,2,74)  (B,T,2,57)
            predict_content = model(T_main_mels, T_other_mels, T_pose,Nl_pose,Nl_content)  # (B*T,2,57)
...
if __name__ == '__main__':
    ...
    model = Landmark_generator_FF(T,d_model,nlayers,nhead,dim_feedforward,dropout)
    ...
    while global_epoch < 9999999999:
        ...
        for step, (T_main_mels, T_other_mels, T_pose, T_content, Nl_pose, Nl_content) in prog_bar:
            ...
            T_main_mels, T_other_mels, T_pose,T_content,Nl_pose,Nl_content= T_main_mels.cuda(non_blocking=True), T_other_mels.cuda(non_blocking=True), T_pose.cuda(non_blocking=True), \
                T_content.cuda(non_blocking=True), Nl_pose.cuda(non_blocking=True),Nl_content.cuda(non_blocking=True)
            #(B,T,1,hv,wv) (B,T,2,74)  (B,T,2,57)
            model.train()
            optimizer.zero_grad()
            predict_content=model(T_main_mels, T_other_mels, T_pose, Nl_pose, Nl_content)  #(B*T,2,57)
