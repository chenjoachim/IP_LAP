...
parser.add_argument("--data_root", type=str,help="Root folder of the dataset", required=True)
...
def process_audio_file(vfile, args):
    ...
    dirname = vfile.split('/')[-3:-1]
    fulldir = path.join(args.out_root, dirname[0], dirname[1])
    os.makedirs(fulldir, exist_ok=True)
    wavpath = path.join(fulldir, f'{vidname}.wav')

    command = template.format(vfile.replace(' ', r'\ '), wavpath.replace(' ', r'\ '))
    subprocess.run(command, shell=True)
    wav = audio.load_wav(wavpath, sample_rate)
    orig_mel = audio.melspectrogram(wav).T
    np.save(path.join(fulldir, vidname), orig_mel)
